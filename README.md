...
2. 进入首页，在 `保存 Apple 登录态` 输入 Apple ID 和密码；也可以点击 `一键粘贴账号`，从剪贴板填充 `账号-密码-手机号-链接`。
3. 点击 `保存旧接口登录态`。
4. 如果 Apple 要求 2FA，默认会向受信任手机号发送短信并自动从链接取码；也可以手动填入 6 位验证码。
5. 点击 `提交旧接口验证码`。
...
1. 进入首页，在 `保存 Apple 登录态` 输入 Apple ID 和密码；也可以点击 `一键粘贴账号`，从剪贴板填充 `账号-密码-手机号-链接`。
2. 点击 `保存新接口登录态`。
3. Apple 返回需要 2FA 时，默认向受信任手机号发送短信验证码，并从短信链接自动取码；收到 6 位验证码后也可以手动填入面板。
4. 点击 `提交新接口验证码`。
...
Apple 登录区域的 `一键粘贴账号` 按钮支持从剪贴板读取以下格式，并自动填充 Apple ID、密码和短信链接：

```text
账号-密码-手机号-链接
```

新接口和旧接口的验证码方式默认都是短信；仍可在下拉框中手动切换为受信任设备。
